# ciq-ha

[HomeAssistant](https://home-assistant.io) plugin for Garmin [ConnectIQ BluetoothLE](https://developer.garmin.com/connect-iq/api-docs/Toybox/BluetoothLowEnergy.html) devices.

# Resources

## Home Assistant
* [Tutorial: Making Your First Add-on](https://developers.home-assistant.io/docs/hassio_addon_tutorial)
* [Home Assistant Add-On Configuration](https://developers.home-assistant.io/docs/hassio_addon_config)

## Garmin
* [Getting Started with Connect IQ BLE Development](https://forums.garmin.com/developer/connect-iq/w/wiki/19/getting-started-with-connect-iq-ble-development)
* [Would you like some Raspberry Pi with your Connect IQ?](https://forums.garmin.com/developer/connect-iq/b/news-announcements/posts/would-you-like-some-raspberry-pi-with-your-connect-iq)
* [Would you like some Pi with your app?](https://forums.garmin.com/developer/connect-iq/f/discussion/164904/would-you-like-some-pi-with-your-app)
* [Garmin Programmers Guide](https://developer.garmin.com/connect-iq/programmers-guide)
* [Garmin API Docs](https://developer.garmin.com/connect-iq/api-docs/)
* [Adding Bluetooth Connectivity to your Connect IQ Projects](https://www.youtube.com/watch?v=drZ_pzGwo7I)

## Nordic Semiconductor
* [nRF5 SDK Getting Started](https://www.nordicsemi.com/Software-and-tools/Software/nRF5-SDK/GetStarted)
* [nRF Connect SDK Docs](https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/nrf/gs_installing.html)
* [nRF Tools](https://infocenter.nordicsemi.com/topic/struct_nrftools/struct/nrftools.html)
* [Zephyr Supported Boards](https://docs.zephyrproject.org/latest/boards/index.html)

## Dependencies
* [abandonware/bleno](https://github.com/abandonware/bleno)
* [abandonware/bluetooth-hci-socket](https://github.com/abandonware/node-bluetooth-hci-socket)
* [node-fetch](https://github.com/node-fetch/node-fetch#fetchurl-options)
